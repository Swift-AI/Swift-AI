...
extension HandwritingViewController {
    ...
    fileprivate func classifyImage() {
        ...
        do {
            ...
            if var (label, confidence) = self.outputToLabel(output) {
                self.updateOutputLabels("\(label)", confidence: "\((confidence).toString(decimalPlaces: 2))%")
            } else {
    ...
    fileprivate func scaleImageToSize(_ image: UIImage, maxLength: CGFloat) -> UIImage {
        ...
        let newImage = UIImage(cgImage: newImageRef, scale: 1.0, orientation: .up)
            //UIImage(ciImage: newImageRef!, scale: 1.0, orientation: .faceUp)
            
            
            //UIImage(CGImage: newImageRef, scale: 1.0, orientation: UIImageOrientation.Up)
