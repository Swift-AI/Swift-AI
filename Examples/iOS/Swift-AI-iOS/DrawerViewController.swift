...
class DrawerViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = DrawerCell()
        cell.titleLabel.text = self.viewControllers[(indexPath as NSIndexPath).row].title!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        DrawerNavigationController.globalDrawerController().switchToViewAtIndex((indexPath as NSIndexPath).row, completion: nil)
